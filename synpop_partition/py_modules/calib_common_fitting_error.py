"""Common functions for the calibration pipeline using fitting error."""
...
class CalibrationSetup:
    ...
    # Path to the calibration data csv file
...
class CalibrationConfig:
    ...
    # Optimizer: mininum evaluations
    opt_min_evals: int

    # Optimizer: maximum evaluations
    opt_max_evals: int

    # Optimizer: number of evaluations to improve over
    opt_last_k: int

    # Optimizer: minimum improvement over last k
    min_last_k_improvement: float
...
def add_calib_task(
    ...
    sbatch_env = {
        ...
        "TRANSMISSIBILITY": transmissibility,
        "REPLICATE": replicate,
        "CALIBRATION_CONFIG_FILE": cfg.setups[setup].calibration_config_file,
        "CALIBRATION_DATA_FILE": cfg.setups[setup].calibration_data_file,
        "LOCATION": state,
        "OUTPUT_DIR": output_dir,
    }
...
def check_job_successful(task_id: str, task_meta: dict) -> bool:
    ...
    if not check_epihiper_successful(output_dir):
        return False
    fitting_error = get_fitting_error(output_dir)
...
def process_all_completed(
    ...
    for _, task_meta in completed_tasks:
        ...
        y = get_fitting_error(output_dir)
        assert y is not None, f"Failed to get error from {output_dir}"
    ...
    group = f"{setup}:{state}"
    msm = load_minimizer(con, group, ManualErrorMinimizer)
    ...
    try:
        ...
        if ys:
            ...
            if msm.eval_cache:
                mean_y = max(y for _, y in msm.eval_cache)
            else:
                mean_y = sys.float_info.max
        ...
        try:
            ...
            log.info(
                "Calibration success: setup=%s, state=%s, transmissibility=%s, error=%s",
                setup,
                state,
                e.x,
                e.y,
            ...
            log.warning(
                "Calibration failed: setup=%s, state=%s, transmissibility=%s, error=%s",
                setup,
                state,
                e.x,
                e.y,
...
@click.group()
def cli():
    """Fitting Error Calibration Pipeline."""
    ...
    with con:
        for setup in cfg.setups:
            for state, synpop in zip(states, synpops):
                ...
                msm = ManualErrorMinimizer(
                    min_x=cfg.tau_range[0],
                    max_x=cfg.tau_range[1],
                    min_evals=cfg.opt_min_evals,
                    max_evals=cfg.opt_max_evals,
                    last_k=cfg.opt_last_k,
                    min_last_k_improvement=cfg.min_last_k_improvement,
                )
                first_x = msm.next_x()
        ...
        minimizers = load_all_minimizers(con, ManualErrorMinimizer)
...
def fitting_error_calib_main(config: CalibrationConfig) -> None:
